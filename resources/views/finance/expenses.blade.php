        ...
        <div class="col-12 col-md-8">
            <div class="d-flex align-items-center gap-2 mb-1">
                <span class="badge rounded-pill bg-white bg-opacity-20 text-white fs-8 px-2.5 py-1">
                    <i class="bi bi-receipt-cutoff me-1"></i> Corporate Outflows
                </span>
                <span class="fs-8 text-white-50">• ${{ number_format($stats['total_expenses']) }} Cumulative Expenses</span>
            </div>
            <h3 class="mb-1 fw-extrabold text-white" style="letter-spacing: -0.02em;">
                Corporate Expenses & Employee Claims
            </h3>
            <p class="mb-0 text-white-50 fs-7">
                Log corporate expenditure, manage travel reimbursements, and approve claim requests.
            </p>
        </div>
        <div class="col-12 col-md-4 text-md-end">
            <button class="btn btn-light rounded-pill px-4 py-2.5 fw-bold text-amber shadow-sm" data-bs-toggle="modal" data-bs-target="#createExpenseModal" style="color: #D97706;">
                <i class="bi bi-plus-circle-fill me-1.5 fs-6"></i> Add Expense Claim
            </button>
        </div>
    </div>
</div>
...
<div class="row g-3 mb-4">
    <!-- Card 1: Total Expenses (Soft Amber) -->
    <div class="col-12 col-sm-6 col-xl-3">
        <div class="pastel-ui8-card card-pastel-amber">
            <div>
                <div class="d-flex justify-content-between align-items-center mb-1">
                    <span class="fs-8 text-secondary fw-semibold">
                        <i class="bi bi-currency-dollar me-1"></i> Total Outflow
                    </span>
                    <span class="ui8-pill-val" style="color: #D97706;">
                        ${{ number_format($stats['total_expenses']) }}
                    </span>
                </div>
                <h4 class="ui8-card-title">Total Corporate Expenses</h4>
                <div class="ui8-card-sub mb-3">
                    <i class="bi bi-building me-1 opacity-75"></i> All Logged Expenses
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-center pt-2">
                ...
                <div class="d-flex gap-1">
                    ...
                    <span class="ui8-tag-chip">#Outflow</span>
                </div>
            </div>
        </div>
    </div>

    <!-- Card 2: Approved Claims (Soft Emerald) -->
    <div class="col-12 col-sm-6 col-xl-3">
            ...
            <div>
                <div class="d-flex justify-content-between align-items-center mb-1">
                    ...
                    <span class="ui8-pill-val" style="color: #059669;">
                        ${{ number_format($stats['approved_expenses']) }}
                    </span>
                </div>
                <h4 class="ui8-card-title">Approved Claims</h4>
                <div class="ui8-card-sub mb-3">
                    <i class="bi bi-building me-1 opacity-75"></i> Cleared & Paid Claims
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-center pt-2">
                <div class="d-flex align-items-center">
    ...
    <!-- Card 3: Pending Approvals (Soft Purple) -->
    <div class="col-12 col-sm-6 col-xl-3">
        <div class="pastel-ui8-card card-pastel-purple">
            <div>
                ...
                <h4 class="ui8-card-title">Pending Approvals</h4>
                <div class="ui8-card-sub mb-3">
                    <i class="bi bi-building me-1 opacity-75"></i> Awaiting Management Approval
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-center pt-2">
                <div class="d-flex align-items-center">
            ...
            <div>
                <div class="d-flex justify-content-between align-items-center mb-1">
                    ...
                    <span class="ui8-pill-val" style="color: #0284C7;">
                        {{ $stats['rejected_count'] }} Claims
                    </span>
                </div>
                <h4 class="ui8-card-title">Rejected Claims</h4>
                <div class="ui8-card-sub mb-3">
                    <i class="bi bi-building me-1 opacity-75"></i> Declined Expense Requests
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-center pt-2">
                <div class="d-flex align-items-center">
                ...
                <div class="d-flex gap-1">
                    <span class="ui8-tag-chip">#Rejected</span>
                    <span class="ui8-tag-chip">#Declined</span>
                </div>
            </div>
        </div>
    </div>
...
<div class="directory-card">
    <div class="p-3 border-bottom d-flex justify-content-between align-items-center bg-light bg-opacity-50">
        <div class="fs-8 text-muted fw-bold">
            Showing <strong class="text-dark">{{ $expenses->firstItem() ?? 0 }} - {{ $expenses->lastItem() ?? 0 }}</strong> of <strong class="text-dark">{{ $expenses->total() }}</strong> Expense Claims
        </div>
        <form method="GET" action="{{ route('finance.expenses.index') }}" class="d-flex gap-2">
        ...
        <table class="table table-directory align-middle mb-0 fs-7">
            ...
            <tbody>
                @forelse($expenses as $e)
                    <tr>
                        <td>
                            <div class="fw-bold text-dark fs-7">{{ $e->category }}</div>
                        </td>
                        <td>
                            <div class="fw-bold text-dark fs-8">{{ $e->employee?->full_name ?? 'Corporate Outflow' }}</div>
                            <div class="fs-8 text-muted">{{ $e->employee?->department?->name }}</div>
                        </td>
                        ...
                        <td>
                            <form action="{{ route('finance.expenses.status', $e->id) }}" method="POST" class="d-inline">
                                @csrf
                                <select name="status" class="form-select form-select-sm rounded-pill fs-8 d-inline-block w-auto" onchange="this.form.submit()">
                                    ...
                                    <option value="approved" {{ $e->status == 'approved' ? 'selected' : '' }}>Approved Claim</option>
                                    <option value="rejected" {{ $e->status == 'rejected' ? 'selected' : '' }}>Rejected Claim</option>
                                </select>
                            </form>
                        </td>
                        <td class="text-end pe-3">
                            <form action="{{ route('finance.expenses.destroy', $e->id) }}" method="POST" onsubmit="event.preventDefault(); confirmDeleteExpense('{{ $e->id }}', '{{ addslashes($e->category) }}', this);">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-light rounded-circle text-danger" title="Delete Expense">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-5 text-muted fs-7">
                            <i class="bi bi-receipt-cutoff fs-2 d-block mb-2 text-slate-300"></i>
                            <div class="fw-bold text-dark">No corporate expenses recorded</div>
                            <p class="fs-8 text-muted mb-3">Click "Add Expense Claim" to submit expenditure entries.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if($expenses->hasPages())
        <div class="p-3 border-top bg-light d-flex justify-content-between align-items-center">
            <div class="fs-8 text-muted">Showing {{ $expenses->firstItem() }} to {{ $expenses->lastItem() }} of {{ $expenses->total() }} entries</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow-lg">
            <div class="modal-header border-bottom px-4 py-3">
                <h5 class="modal-title fw-bold fs-6 text-dark">Record Expense Claim</h5>
            ...
            <form action="{{ route('finance.expenses.store') }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-bold fs-7 text-dark">Expense Category <span class="text-danger">*</span></label>
                        <input type="text" name="category" class="form-control rounded-3 fs-8" placeholder="e.g. Cloud Infrastructure & Hosting" required>
                    </div>

                    <div class="row g-2 mb-3">
                        <div class="col-6">
                            <label class="form-label fw-bold fs-7 text-dark">Amount ($) <span class="text-danger">*</span></label>
                            <input type="number" step="0.01" name="amount" class="form-control rounded-3 fs-8" placeholder="150.00" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-bold fs-7 text-dark">Expense Date <span class="text-danger">*</span></label>
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-6">
                            <label class="form-label fw-bold fs-7 text-dark">Claimant Employee</label>
                            <select name="employee_id" class="form-select rounded-3 fs-8">
                                <option value="">Corporate Outflow</option>
                        ...
                        <div class="col-6">
                            ...
                            <select name="status" class="form-select rounded-3 fs-8">
                                ...
                                <option value="approved">Approved Claim</option>
                            </select>
                        </div>
                    </div>
                </div>
                <div class="modal-footer border-top px-4 py-3">
                    ...
                    <button type="submit" class="btn btn-amber rounded-pill px-4 fs-8 fw-bold text-white" style="background: #D97706; border: none;">Save Expense Claim</button>
                </div>
            </form>
        </div>
    </div>
...
@push('scripts')
<script>
    function confirmDeleteExpense(id, category, formEl) {
        const isDark = document.documentElement.getAttribute('data-bs-theme') === 'dark';
        
        Swal.fire({
            title: `<div class="d-flex align-items-center justify-content-center gap-2 text-danger fw-bold fs-5 mb-1">
                        <i class="bi bi-exclamation-triangle-fill fs-4"></i> Delete Expense Entry?
                    </div>`,
            html: `
                <div class="text-center py-2">
                    <p class="fs-7 text-secondary mb-3" style="line-height: 1.6;">
                        Are you sure you want to delete expense <strong class="text-dark">${category}</strong>?
                    </p>
                    <div class="alert alert-danger border-0 fs-8 py-2.5 px-3 text-start mb-0 rounded-3" style="background: ${isDark ? '#374151' : '#FEF2F2'}; color: ${isDark ? '#F87171' : '#991B1B'};">
                        ...
                        Deleting this expense will remove it from financial reports.
                    </div>
                </div>
            `,
            showCancelButton: true,
            confirmButtonText: '<i class="bi bi-trash-fill me-1"></i> Yes, Delete Expense',
