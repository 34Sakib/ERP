        ...
        <div class="col-12 col-md-8">
            <div class="d-flex align-items-center gap-2 mb-1">
                <span class="badge rounded-pill bg-white bg-opacity-20 text-white fs-8 px-2.5 py-1">
                    <i class="bi bi-journal-bookmark-fill me-1"></i> General Ledger
                </span>
                <span class="fs-8 text-white-50">• {{ $stats['total_accounts'] }} Active Ledger Accounts</span>
            </div>
            <h3 class="mb-1 fw-extrabold text-white" style="letter-spacing: -0.02em;">
                Chart of Accounts & General Ledger
            </h3>
            <p class="mb-0 text-white-50 fs-7">
                Manage asset accounts, liabilities, equity, revenue income, and corporate expenses.
            </p>
        </div>
        <div class="col-12 col-md-4 text-md-end">
            <button class="btn btn-light rounded-pill px-4 py-2.5 fw-bold text-indigo shadow-sm" data-bs-toggle="modal" data-bs-target="#createAccountModal" style="color: #4F46E5;">
                <i class="bi bi-plus-circle-fill me-1.5 fs-6"></i> Add Ledger Account
            </button>
        </div>
    </div>
</div>
...
<div class="row g-3 mb-4">
    <!-- Card 1: Total Accounts (Soft Sky Blue) -->
    <div class="col-12 col-sm-6 col-xl-3">
        <div class="pastel-ui8-card card-pastel-indigo">
            <div>
                <div class="d-flex justify-content-between align-items-center mb-1">
                    <span class="fs-8 text-secondary fw-semibold">
                        <i class="bi bi-journal-text me-1"></i> Ledger Accounts
                    </span>
                    <span class="ui8-pill-val" style="color: #0284C7;">
                        {{ $stats['total_accounts'] }} Accounts
                    </span>
                </div>
                <h4 class="ui8-card-title">Total Ledger Accounts</h4>
                <div class="ui8-card-sub mb-3">
                    <i class="bi bi-building me-1 opacity-75"></i> General Ledger Master
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-center pt-2">
                ...
                <div class="d-flex gap-1">
                    <span class="ui8-tag-chip">#Accounts</span>
                    <span class="ui8-tag-chip">#Ledger</span>
                </div>
            </div>
        </div>
    </div>

    <!-- Card 2: Asset Accounts (Soft Emerald) -->
    <div class="col-12 col-sm-6 col-xl-3">
        <div class="pastel-ui8-card card-pastel-emerald">
            <div>
                ...
                <h4 class="ui8-card-title">Asset Accounts</h4>
                <div class="ui8-card-sub mb-3">
                    <i class="bi bi-building me-1 opacity-75"></i> Cash & Receivables
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-center pt-2">
                <div class="d-flex align-items-center">
                ...
                <div class="d-flex gap-1">
                    <span class="ui8-tag-chip">#Assets</span>
                    <span class="ui8-tag-chip">#Cash</span>
                </div>
            </div>
        </div>
    </div>

    <!-- Card 3: Income Accounts (Soft Purple) -->
    <div class="col-12 col-sm-6 col-xl-3">
            ...
            <div>
                <div class="d-flex justify-content-between align-items-center mb-1">
                    ...
                    <span class="ui8-pill-val" style="color: #7C3AED;">
                        {{ $stats['income_count'] }} Income
                    </span>
                </div>
                <h4 class="ui8-card-title">Income Accounts</h4>
                <div class="ui8-card-sub mb-3">
                    <i class="bi bi-building me-1 opacity-75"></i> Revenue & Sales
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-center pt-2">
                <div class="d-flex align-items-center">
                ...
                <div class="d-flex gap-1">
                    <span class="ui8-tag-chip">#Income</span>
    ...
    <!-- Card 4: Expense Accounts (Soft Amber) -->
    <div class="col-12 col-sm-6 col-xl-3">
            ...
            <div>
                <div class="d-flex justify-content-between align-items-center mb-1">
                    ...
                    <span class="ui8-pill-val" style="color: #D97706;">
                        {{ $stats['expense_count'] }} Expenses
                    </span>
                </div>
                <h4 class="ui8-card-title">Expense Accounts</h4>
                <div class="ui8-card-sub mb-3">
                    <i class="bi bi-building me-1 opacity-75"></i> Operational Outflows
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-center pt-2">
                <div class="d-flex align-items-center">
                ...
                <div class="d-flex gap-1">
                    ...
                    <span class="ui8-tag-chip">#Outflows</span>
                </div>
            </div>
        </div>
    </div>
...
<div class="directory-card">
    <div class="p-3 border-bottom d-flex justify-content-between align-items-center bg-light bg-opacity-50">
        <div class="fs-8 text-muted fw-bold">
            Showing <strong class="text-dark">{{ $accounts->firstItem() ?? 0 }} - {{ $accounts->lastItem() ?? 0 }}</strong> of <strong class="text-dark">{{ $accounts->total() }}</strong> Ledger Accounts
        </div>
        <form method="GET" action="{{ route('finance.accounts.index') }}" class="d-flex gap-2">
            <input type="text" name="search" class="form-control rounded-pill fs-8 ps-3" value="{{ request('search') }}" placeholder="Search account name...">
        </form>
    </div>
        ...
        <table class="table table-directory align-middle mb-0 fs-7">
            ...
            <tbody>
                @forelse($accounts as $acc)
                    <tr>
                        <td>
                            <div class="fw-bold text-dark fs-7">{{ $acc->name }}</div>
                        </td>
                        <td>
                        ...
                        <td class="text-end pe-3">
                            <div class="d-flex justify-content-end align-items-center gap-1.5">
                                <button class="btn btn-sm btn-light rounded-circle text-primary" 
                                        onclick="editAccountModal('{{ $acc->id }}', '{{ addslashes($acc->name) }}', '{{ $acc->type }}')"
                                        title="Edit Account">
                                    <i class="bi bi-pencil-fill fs-8"></i>
                                </button>
                                <form action="{{ route('finance.accounts.destroy', $acc->id) }}" method="POST" onsubmit="event.preventDefault(); confirmDeleteAccount('{{ $acc->id }}', '{{ addslashes($acc->name) }}', this);">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-light rounded-circle text-danger" title="Delete Account">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="text-center py-5 text-muted fs-7">
                            <i class="bi bi-journal-bookmark fs-2 d-block mb-2 text-slate-300"></i>
                            <div class="fw-bold text-dark">No ledger accounts registered</div>
                            <p class="fs-8 text-muted mb-3">Click "Add Ledger Account" to configure the Chart of Accounts.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if($accounts->hasPages())
        <div class="p-3 border-top bg-light d-flex justify-content-between align-items-center">
            <div class="fs-8 text-muted">Showing {{ $accounts->firstItem() }} to {{ $accounts->lastItem() }} of {{ $accounts->total() }} entries</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow-lg">
            <div class="modal-header border-bottom px-4 py-3">
                <h5 class="modal-title fw-bold fs-6 text-dark" id="accountModalTitle">Add Ledger Account</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('finance.accounts.store') }}" method="POST" id="accountForm">
                @csrf
                <input type="hidden" name="_method" id="accountMethod" value="POST">
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-bold fs-7 text-dark">Account Name & Code <span class="text-danger">*</span></label>
                        <input type="text" name="name" id="acc_name" class="form-control rounded-3 fs-8" placeholder="e.g. 1010 - Operating Cash Account" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold fs-7 text-dark">Account Type <span class="text-danger">*</span></label>
                        <select name="type" id="acc_type" class="form-select rounded-3 fs-8" required>
                ...
                <div class="modal-footer border-top px-4 py-3">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-4 fs-8 fw-bold" style="background: #4F46E5; border: none;">Save Account</button>
                </div>
            </form>
        </div>
    </div>
...
@push('scripts')
<script>
    function editAccountModal(id, name, type) {
        document.getElementById('accountModalTitle').textContent = 'Edit Ledger Account';
        document.getElementById('accountForm').action = "{{ url('finance/accounts') }}/" + id;
        document.getElementById('accountMethod').value = 'PUT';
            ...
            html: `
                <div class="text-center py-2">
                    ...
                    <div class="alert alert-danger border-0 fs-8 py-2.5 px-3 text-start mb-0 rounded-3" style="background: ${isDark ? '#374151' : '#FEF2F2'}; color: ${isDark ? '#F87171' : '#991B1B'};">
                        ...
                        Deleting this account will remove it from the Chart of Accounts ledger.
                    </div>
                </div>
            `,
